...
export const UserOrders = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto min-h-[100vh] p-4">
      {isNewOrderPlaced && (
      ...
      <div className="flex flex-col gap-4 mt-8">
        {displayedOrders
          .map((order) => (
            <div
              className="border border-gray-300 rounded-lg p-4 flex flex-wrap  items-start"
              key={order._id}
            >
              <div className="flex-1 pr-4">
                {order.order.map((product, index) => (
                  <div key={product.name} className="">
                    <p className="font-bold capitalize mb-1">{product.name}</p>
